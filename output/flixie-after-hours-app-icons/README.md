# Flixie After Hours — app icon pack

Exact editable F geometry from the After Hours direction, including the detached lower-right bar. Lilac **#B9A0FF** on midnight **#120A24**. Prepared for the existing Flutter iPhone/iPad and Android project. The Flixie wordmark is unchanged.
...
The supplied pack is self-contained under `output/flixie-after-hours-app-icons`. It has not been copied over the current native app resources. Installation is the folder merge/replacement above. Existing splash artwork is a separate branding surface and is not replaced by this icon pack.

## Validation

- SVG-derived exports preserve the exact source paths; only centring, scale and colour vary by platform.
